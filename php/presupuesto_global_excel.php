...
$objPHPExcel->getActiveSheet()->getStyle("A8:ZZ3")->getFont()->getColor()->applyFromArray(
	...
	$sql = "SELECT c.id, c.colegio FROM presupuestos p JOIN colegios c ON c.id=p.id_colegio WHERE p.id_periodo='".$_POST["periodo"]."' AND p.aprobado=1 GROUP BY c.id";
	$req = $bdd->prepare($sql);
	$req->execute();
...
foreach($colegios as $colegio) {
		...
		$conta_cols2=$conta_cols;
		$conta_cols3 = array_search($cols[$conta_cols], $cols2);
		$conta_cols3 = $conta_cols3 +1;
		$conta_cols4 = $conta_cols3 +1;

		
		$objPHPExcel->getActiveSheet()->SetCellValue($cols[$conta_cols]."8", "$colegio[colegio]");
