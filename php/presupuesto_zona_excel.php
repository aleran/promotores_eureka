...
$sql = "SELECT c.id, c.colegio, p.fila FROM presupuestos p JOIN colegios c ON c.id=p.id_colegio WHERE p.id_periodo='".$_POST["periodo"]."' AND c.cod_zona='".$_POST["zona"]."' AND (p.aprobado=1 OR p.definido=1) GROUP BY c.id ORDER BY p.fila";
  $req = $bdd->prepare($sql);
  $req->execute();
