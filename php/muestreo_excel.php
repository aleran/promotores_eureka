...
$sql = "SELECT UPPER(l.libro) as libro, l.id FROM muestreos m JOIN libros_muestreos lm ON lm.cod_muestreo=m.codigo JOIN libros l ON l.id=lm.id_libro WHERE m.id_usuario='".$_POST["promo"]."' AND m.id_periodo='".$_POST["periodo"]."' GROUP BY l.id";
...
foreach($muestras as $muestra) {
	...
	$sql = "SELECT COUNT(id_libro)  as cantidad FROM `libros_muestreos` lm JOIN muestreos m ON m.codigo=lm.cod_muestreo WHERE id_libro='".$muestra["id"]."' AND id_usuario='".$_POST["promo"]."'";
...
header('Content-Disposition: attachment; filename="Reporte_muestreo.xlsx"');
