						...
						<div class="row">
							<div class="col-xs-12">
								...
								<?php 
                                	...
                                	$sql = "SELECT p.id, z.zona, u.nombres, u.apellidos, p.fecha, c.colegio, e.estado FROM pedidos p JOIN colegios c ON p.id_colegio=c.id JOIN zonas z ON z.codigo=c.cod_zona JOIN usuarios u ON u.cod_zona=z.codigo JOIN estados_pedidos e ON e.id=p.estado  WHERE p.id_usuario='".$_SESSION["id"]."' GROUP BY p.id ";
									$req = $bdd->prepare($sql);
									$req->execute();
